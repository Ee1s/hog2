...
struct WMMCompare {
	...
	bool operator()(const AStarOpenClosedData<state> &i1, const AStarOpenClosedData<state> &i2) const
	{
		...
		double p2 = std::max(i2.g+i2.h, i2.g*lambda);
//		double p1 = i1.g+i1.h;
//		double p2 = i2.g+i2.h;
		if (fequal(p1, p2))
		{
...
public:
	...
	virtual const char *GetName() { return "WMM"; }
...
template <class state, class action, class environment, class priorityQueue>
void WMM<state, action, environment, priorityQueue>::GetPath(environment *env, const state& from, const state& to,
			 Heuristic<state> *forward, Heuristic<state> *backward, std::vector<state> &thePath)
{
...
template <class state, class action, class environment, class priorityQueue>
bool WMM<state, action, environment, priorityQueue>::InitializeSearch(environment *env, const state& from, const state& to,
...
template <class state, class action, class environment, class priorityQueue>
bool WMM<state, action, environment, priorityQueue>::DoSingleSearchStep(std::vector<state> &thePath)
{
	...
	double p1 = std::max(nextForward.g+nextForward.h, nextForward.g*lambda1);
	double p2 = std::max(nextBackward.g+nextBackward.h, nextBackward.g*lambda2);
	...
	if (fless(currentCost, DBL_MAX))
	{
		...
		}
		{
			...
			backwardP = std::max(iB.g+iB.h, iB.g*lambda2);
			auto iF = forwardQueue.Lookat(forwardQueue.Peek());
			forwardP = std::max(iF.g+iF.h, iF.g*lambda1);
		}
		bool done = false;
...
template <class state, class action, class environment, class priorityQueue>
void WMM<state, action, environment, priorityQueue>::Expand(priorityQueue &current,
														   priorityQueue &opposite,
														   Heuristic<state> *heuristic, const state &target)
{
...
template <class state, class action, class environment, class priorityQueue>
void WMM<state, action, environment, priorityQueue>::OpenGLDraw() const
{
...
template <class state, class action, class environment, class priorityQueue>
void WMM<state, action, environment, priorityQueue>::OpenGLDraw(const priorityQueue &queue) const
{
