...
enum stateLocation {
	kOpenReady,//priority queue 0, low g -> low f
	kOpenWaiting,//priority queue 1, low f -> low g
	kClosed,
	kUnseen
};
...
public:
	...
	size_t OpenReadySize() const { return priorityQueues[0].size(); }
	size_t OpenWaitingSize() const { return priorityQueues[1].size(); }
	size_t OpenSize() const { return priorityQueues[0].size()+priorityQueues[1].size(); }

	size_t ClosedSize() const { return size()-OpenReadySize()-OpenWaitingSize(); }
	size_t size() const { return elements.size(); }
	//	void verifyData();
	bool Validate0(int index = 0)
	{
		...
		if (priorityQueues[whichQ].size() > child2 &&
			...
			return false;
		return Validate0(child1) && Validate0(child2);
	}

	bool Validate1(int index = 0)
	{
		...
		if (priorityQueues[whichQ].size() > child2 &&
			!compare(elements[priorityQueues[whichQ][child2]], elements[priorityQueues[whichQ][index]]))
			return false;
		return Validate1(child1) && Validate1(child2);
	}
private:
...
uint64_t BDOpenClosed<state, CmpKey0, CmpKey1, dataStructure>::PutToReady()
{
	...
	uint64_t ans = priorityQueues[1][0];
	elements[ans].where = kOpenReady;
	priorityQueues[1][0] = priorityQueues[1].back();
	elements[priorityQueues[1][0]].openLocation = 0;
	priorityQueues[1].pop_back();

	HeapifyDown(0, kOpenWaiting);

	//put it to openReady
	priorityQueues[0].push_back(ans);
	elements[ans].openLocation = priorityQueues[0].size()-1;
	HeapifyUp(priorityQueues[0].size() - 1,kOpenReady);
...
//	elements[objKey].openLocation = waitingQueue.size();
//	waitingQueue.push_back(objKey);
//	HeapifyUp(waitingQueue.size()-1);
//}
...
// */
//template<typename state, typename CmpKey0, typename CmpKey1,   class dataStructure>
//OBJ BDOpenClosed<state, CmpKey0, CmpKey1,   dataStructure>::find(OBJ val)
//{
//	if (!IsIn(val))
//		return OBJ();
//    return waitingQueue[table[val]];
//}

///**
// * Returns true if no items are in the BDOpenClosed.
// */
//template<typename state, typename CmpKey0, typename CmpKey1,   class dataStructure>
//bool BDOpenClosed<state, CmpKey0, CmpKey1,   dataStructure>::Empty()
//{
...
//{
//	assert(waitingQueue.size() == table.size());
//	BDOpenClosed::IndexTable::iterator iter;
//	for (iter = table.begin(); iter != table.end(); iter++)
//	{
//		EqKey eq;
//		assert(eq(iter->first, waitingQueue[iter->second])); 
//	}
//}
